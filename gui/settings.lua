...
function showSettings(player, index)
    ...
    if global.guiVisible[index] == 0 and player.gui[guiPos].logisticsFrame ~= nil then
        ...
        -- logistics data
        settingsTable.add({type = "label", name = "logDataLabel", caption = {"settings.log-data"}, style = "als_info_label"})
        local settingFlow = settingsTable.add({type = "flow", name = "logDataFlow", direction = "horizontal"})
        settingFlow.add({type = "button", name = "updateLogisticsData", caption = {"settings.log-data-update"}, style = "als_button"})
